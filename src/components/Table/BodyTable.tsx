      ...
      <thead>
        <tr>
          {tableProps.structure.map((curBase: Column) =>
            renderCellHeader(curBase, sortColumn)
          )}
          {isEditable && (
            <>
              ...
              {tableProps.structure.map((currentColumn: Column) => (
                <td
                  ...
                  style={{
                    ...
                    fontSize: currentColumn.fontSize,
                    height: currentColumn.height,
                  }}
                >
                  {currentColumn.element
                    ? currentColumn.element(currentData)
                    : currentData[currentColumn.path!]}
                </td>
              ))}
              {isEditable && (
                <>
                  <td key={"edit"} style={{ width: "110px", fontSize: "20px" }}>
                    <button
                      ...
                      onClick={() => updateColumn(currentData)}
                    >
                      EDIT
                    </button>
                  </td>
                  <td
                    key={"delete"}
                    style={{ width: "110px", fontSize: "20px" }}
                  >
                    <button
  ...
  function renderCellHeader(column: Column, currentSort: sortColumnProps) {
    ...
    return (
      <th
        ...
        onClick={() => {
          handleSortColumn(column.path, currentSort.order);
        }}
      >
        {column.label}{" "}
        {currentSort.path == column.path && renderIcon(currentSort.order)}
      </th>
    );
  }
