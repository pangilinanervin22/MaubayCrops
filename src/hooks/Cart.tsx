...
export function useGetCartList(accountId: string) {
  ...
  useEffect(() => {
    ...
    const cartUnsubscribe = onSnapshot(
      collection(firebaseDB, `/accounts/${accountId}/cart`),
      async (snapshot) => {
        const cartDocs = snapshot.docs;

        const newCartItems = await Promise.all(
          cartDocs.map(async (currentDoc) => {
  ...
  const addToCart = async (product: Product) => {
    try {
      ...
      if (productSnap.data().quantity < 1) {
        await deleteDoc(
          doc(firebaseDB, `/accounts/${accountId}/cart/${product._id}`)
        );
      ...
      const docRef = doc(
        firebaseDB,
        `/accounts/${accountId}/cart/${product._id}`
      );
...
export function useUpdateCart() {
  const updateCart = async (
    ...
    try {
      ...
      const docRef = doc(
        firebaseDB,
        `/accounts/${accountId}/cart/${productId}`
      );
      ...
      if (product.quantity < inputCartItemQuantity) {
        await setDoc(
          docRef,
          { cartItemQuantity: productSnap.data().quantity },
          { merge: true }
        );
        toast("Quantity is set the only available stock", {
          type: "warning",
          toastId: productId,
        });
        return { ok: true, message: "Cart updated" };
      ...
      await setDoc(
        docRef,
        { cartItemQuantity: inputCartItemQuantity },
        { merge: true }
      );
      return { ok: true, message: "Cart updated" };
    } catch (e) {
...
export function useRemoveFromCart(accountId: string) {
  const removeFromCart = async (productId: string) => {
    try {
      await deleteDoc(
        doc(firebaseDB, `/accounts/${accountId}/cart/${productId}`)
      );
      return { ok: true, message: "Product removed from cart" };
    } catch (e) {
